 ...
 * feed yet), flagged rather than silently wired to nothing. Theme toggle is the existing real/
 * functional component (next-themes) reused as-is.
 *
 ...
 *
 * Role badge is a real dropdown when the session holds more than one switchable role (Merchant +
 * Creator) — selecting the other one navigates to its dashboard. A single-role session shows the
 * same badge with no dropdown affordance, since there's nothing real to switch to.
 */
              ...
              {switchableRoles.map((role) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (role !== "merchant") router.push(ROLE_PREFIX[role as "creator"]);
                  }}
                  className={cn(
