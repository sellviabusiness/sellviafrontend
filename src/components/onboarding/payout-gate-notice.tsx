 ...
 * Not wired into any Feature 4 screen in this task — Feature 4 (Applications, Discover, etc.) is
 * out of scope here — this is only the shared, ready-to-import component it will consume, so the
...
export function PayoutGateNotice({ message = PAYOUT_INCOMPLETE_MESSAGE }: { message?: string }) {
  return (
    ...
    >
      <AlertCircle className="h-4 w-4 shrink-0 text-accent-foreground" aria-hidden="true" />
      <span>{message}</span>
    </div>
  );
}
