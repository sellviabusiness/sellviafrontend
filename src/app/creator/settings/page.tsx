...
import { UserCircle, CreditCard, ShieldCheck, ChevronRight, Trash2, Moon } from "lucide-react";
...
const SECTIONS = [
  { href: "/creator/settings/profile", icon: UserCircle, title: "Profile", description: "Niche, audience size, and engagement rate." },
  { href: "/creator/settings/payout", icon: CreditCard, title: "Payout method", description: "Connect the account SellVia pays your commission to." },
  { href: "/creator/settings/security", icon: ShieldCheck, title: "Security", description: "Password, two-factor authentication, and active sessions." },
] as const;

/** E8/E9/E10's shared hub, plus F2's Delete Account entry point (Playbook 06) kept visually
 *  separate as a danger-zone card. Grouped into labeled sections (Preferences / Account / Danger
 *  zone) — Profile/Payout/Security render as one bordered list with internal dividers rather than
 *  three separate cards, the common "settings list" pattern. */
export default function CreatorSettingsPage() {
  return (
    <div className="mx-auto max-w-lg space-y-8">
      <div>
        <h1 className="font-[family-name:var(--font-heading)] text-xl font-semibold text-foreground">Settings</h1>
        <p className="text-sm text-muted-foreground">Manage your profile, payout method, and account security.</p>
      </div>
      ...
      <section className="space-y-2">
        ...
        <Link href="/creator/settings/delete-account">
          <Card className="flex items-center gap-4 border-danger-border p-5 transition-colors hover:bg-danger-bg">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">Ends both Creator and Merchant access, with a 14-day cancellable grace period.</p>
            </div>
            <ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground-2" aria-hidden="true" />
          </Card>
        </Link>
